...
## 🚨 IMMEDIATE PRIORITIES (Complete Before Phase 3)
...
- Calculate true marketing ROI (quote → booking rate)
...
## 💰 PHASE 3 FOUNDATION (Revenue Generation MVP)
...
- [ ] Booking workflow: Quote → Customer Details → Payment → Confirmation
- [ ] Payment form using Stripe Elements (PCI compliant)
- [ ] Accepts: Credit/debit cards, Apple Pay, Google Pay
- [ ] Payment success: Booking status → "confirmed", charge customer
...
**Example SMS:**
```
DURDLE-12345 confirmed. Pickup: Bournemouth Station, Dec 10 at 14:30. £28 paid. View: durdle.link/b/abc123
```
...
- [ ] Booking timeline: Created → Confirmed → (Assigned) → (Completed)
...
- [ ] Confirmation modal: "Are you sure? This will refund £X to your card."
- [ ] If >24 hours: Full refund, booking status → "cancelled"
...
## 📊 PHASE 3 ENHANCEMENTS (Important but can wait)
...
- [ ] Conversion funnel: Quotes → Bookings (percentage)
...
## 🚀 PHASE 4 PREPARATION (Future Strategic Work)
...
- [ ] Update job status: En Route → Arrived → Passenger Onboard → Completed
...
**Acceptance Criteria:**
- [ ] E2E test: Customer quote → booking → payment (happy path)
- [ ] E2E test: Quote expiry handling
- [ ] E2E test: Payment failure → retry → success
- [ ] E2E test: Admin creates booking manually
- [ ] E2E test: Admin assigns driver to booking
- [ ] E2E test: Customer cancels booking → refund
...
**Cost:** SNS £0.05/SMS, Twilio £0.08/SMS (40% more expensive but 99.95% delivery rate)
...
- Google Maps API costs stay within budget (<£100/month at 1000 bookings/month)
...
**Approvers:** CTO ✅ | CPO (Pending) | CEO (For Phase 3 budget approval)
